...
.. class:: DataList(choices=None, *, render_kw=None)
    ...
    callable invoked at render time. The callable may take no argument
    (``fn()``) for a static list, or ``(field)`` to compute suggestions
    from the current field value (``field.data``) — convenient for
    ...
    **Anonymous (inline) DataList owned by a field**
    ...
    ``{{ form.country() }}`` alone emits both ``<input list="...">`` and
    ``<datalist id="...">`` as a single functional pair — no extra
    ``{{ form.country.datalist() }}`` call is needed. An inline datalist
    is owned by exactly one field, so its DOM element is always emitted
    next to that field.
    ...
    entry's own value::
        ...
        class F(Form):
            items = FieldList(
                StringField(
                    datalist=DataList(choices=lambda form, field: suggest(field.data))
                ),
                min_entries=3,
            )
    ...
    ``field.datalist()`` remains available if you need the element on
    its own (e.g. to place it elsewhere in the DOM); calling it in
    addition to ``field()`` would produce a duplicate ``<datalist>``.
